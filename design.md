...
```python
artifact = jit(f, **jit_options).compile_artifact(
    *args_or_specs,
...
- Materializes one specialization of a jitted function as a serialized compiled
  artifact.
- Eagerly compiles if the specialization is not already cached.
...
| `jit(...).compile_artifact` | low-level executable serialization | materialize target-specific executable |
