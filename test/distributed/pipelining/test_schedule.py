...
class ScheduleTest(TestCase):
    ...
    def test_pipeline_schedule_multi_allows_nonfirst_stage_without_recv(self):
        class _PeerOnlyOp:
            def __init__(self, peer: int):
                self.peer = peer

        class _NoInputMiddleStage(MockPipelineStage):
            def __init__(self):
                super().__init__(num_stages=3, group_size=3, group_rank=1)
                self.stage_index = 1
            ...
            def _prepare_forward_infra(self, *args, **kwargs):
                # Middle stages can legitimately have no received activations
                # when their module takes no tensor inputs.
                self.args_recv_info = {0: tuple()}
                self.act_send_info = {0: [2]}
                return tuple()
        ...
        schedule = PipelineScheduleMulti([_NoInputMiddleStage()], n_microbatches=1)
        schedule.pipeline_order = {0: [None], 1: [None], 2: [None]}

        # Should not fail validation, because no non-adjacent peers are present.
        schedule.step()
