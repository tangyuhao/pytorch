...
class TestNonstrictJitAgainstJax(TestCase):
    ...
    def test_compile_artifact_static_args_are_bound_at_materialization_time(self):
        ...
        torch_artifact = jit(torch_f, static_argnames="scale").compile_artifact(
            {"x": torch.ones(2)}, scale=2
        )

        self.assertEqual(
            torch_artifact({"x": torch.ones(2)})["y"],
            torch.tensor([2.0, 2.0]),
        )

        with self.assertRaises(TypeError):
            torch_artifact({"x": torch.ones(2)}, scale=2)

    def test_compile_artifact_rejects_non_inductor_backend(self):
        jf = jit(lambda x: x + 1, backend=_identity_backend)
        with self.assertRaisesRegex(NotImplementedError, "backend='inductor'"):
            jf.compile_artifact(torch.ones(2))

    def test_compile_artifact_top_level_helper(self):
        jf = jit(lambda x: x + 1)
        x = torch.ones(2)
        self.assertEqual(compile_artifact(jf, x)(x), torch.tensor([2.0, 2.0]))
    ...
    def test_compile_artifact_save_load(self):
        ...
        x = torch.randn(4)
        torch_artifact = jit(torch_f).compile_artifact(x)
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(loaded(x), torch_f(x))
