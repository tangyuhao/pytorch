...
def jit(
    ...
    *,
    ...
    Example:
        ...
        {"y": tensor([2., 2.])}
        >>> artifact = jf.compile_artifact({"x": torch.ones(2)}, scale=2)
        >>> artifact({"x": torch.ones(2)})
        {"y": tensor([2., 2.])}
    """
    if fn is None:
    ...
    static_order = tuple(name for name in sig.parameters if name in static_names)
    dynamic_order = tuple(name for name in sig.parameters if name not in static_names)
    compile_backend = _backend_fn(backend)
    cache: dict[Any, tuple[Callable[..., Any], pytree.TreeSpec]] = {}
    artifact_cache: dict[Any, CompiledArtifact] = {}
    ...
    def wrapped(*args: Any, **kwargs: Any) -> Any:
        ...
        entry = cache.get(key)
        if entry is None:
            gm, out_spec = _trace(fn, sig, values, dynamic_order, flat_args, in_spec)
            entry = compile_backend(gm, list(flat_args)), out_spec
            cache[key] = entry
    ...
    def compile_artifact(
        ...
        **kwargs: Any,
        ...
        if entry is None:
            ...
            artifact_cache[key] = entry
        return entry

    wrapped.compile_artifact = compile_artifact  # type: ignore[attr-defined]
    return wrapped


def compile_artifact(
    jitted_fn: Callable[..., Any],
    *args: Any,
    target: Target | None = None,
    **kwargs: Any,
) -> CompiledArtifact:
    """Materialize one ``jit`` specialization as a compiled artifact.

    Prefer the method form:

        >>> artifact = jit(f).compile_artifact(torch.ones(2))

    This helper exists so the prototype still has a named ``compile_artifact``
    entrypoint. It produces a target-specific compiled package, not a portable
    graph artifact.
    """
    method = getattr(jitted_fn, "compile_artifact", None)
    if method is None:
        raise TypeError("compile_artifact() expects a function returned by jit()")
    return method(*args, target=target, **kwargs)
